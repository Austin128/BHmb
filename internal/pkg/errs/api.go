...
var (
	...
	ErrPathEscape         = New(CodePathEscape, "路径越出允许范围")
	ErrFileNotFound       = New(CodeFileNotFound, "文件或目录不存在")
	ErrFileExists         = New(CodeFileExists, "目标已存在")
	ErrFilePermission     = New(CodeFilePermission, "没有文件系统权限")
	ErrFileTooLarge       = New(CodeFileTooLarge, "文件超出大小限制")
	ErrChunkChecksum      = New(CodeChunkChecksum, "分片校验失败")
	ErrUploadExpired      = New(CodeUploadExpired, "上传会话已过期")
	ErrArchiveUnsupported = New(CodeArchiveUnsupported, "不支持的压缩格式")
	ErrArchiveCorrupt     = New(CodeArchiveCorrupt, "压缩包损坏或密码错误")
	ErrNotUTF8            = New(CodeNotUTF8, "文件不是文本或编码不受支持")
	ErrFileChanged        = New(CodeFileChanged, "文件已被其他人修改")
	ErrProtectedPath      = New(CodeProtectedPath, "受保护路径禁止操作")
	ErrDiskFull           = New(CodeDiskFull, "磁盘已满")
)
