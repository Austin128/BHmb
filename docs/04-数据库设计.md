...
| 差异点 | MySQL | SQLite | PostgreSQL | 青垣面板 处理方式 |
| --- | --- | --- | --- | --- |
...
SQLite 差异：`TINYINT(1)`→`INTEGER`，`VARCHAR(n)`→`TEXT`，去掉 `ENGINE`/`CHARSET`/`COMMENT`/`ON UPDATE`；唯一索引中 `deleted_at` 为 NULL 时 SQLite 视为互不相等，需改为 `deleted_at_flag` 生成列或在应用层做唯一性预检（青垣面板 采用后者）。
...
| 字段 | 类型 | 必填 | 默认 | 说明 / 枚举取值 |
| --- | --- | --- | --- | --- |
| code | VARCHAR(128) | 是 | 无 | 全局唯一，如 `website:site:create`，全量清单见 4.18.2 |
...
| scope_json | TEXT | 否 | NULL | Schema：`["website:site:list","mon:metric:read"]` |
...
| rate_limit_json | TEXT | 否 | NULL | Schema：`{"enabled":true,"bandwidthKb":512,"burstKb":1024}`；limit_conn/limit_req 需在 http 块声明 zone，面板只纳管 vhost 目录，故暂不暴露 |
| settings_json | TEXT | 否 | NULL | 站点设置聚合。Schema：`{"runPath":"/public","autoIndex":false,"cacheProfile":"day","antiLeech":{...},"accessControl":{...},"redirects":[...]}`；默认文档与日志开关仍落在 `index_files`、`access_log_enabled`、`error_log_enabled` 列上 |

删除网站为敏感操作，需 `website:site:delete` + 二次确认；根目录默认移入回收站而非直接 rm。
...
内置模板正文随面板二进制内置（`internal/service/site/rewrite/*.conf`，以 embed 形式发布），保证无网环境也有规则库；用户可切为 `custom` 自行维护正文。每个站点只保留一条当前生效规则，正文以片段形式落盘到 `<vhost>/rewrite/<site>.conf` 并被站点配置 include。
...
青垣面板 不使用 GORM `AutoMigrate` 作为生产手段（无法表达索引重命名、数据回填、字段收窄等变更），统一采用 **golang-migrate** 风格的顺序 SQL 迁移，并通过 `embed.FS` 打包进 `novapanel` 二进制，实现单文件分发、启动自动升级。
...
| code | 名称 | 模块 | 归属内置角色（admin/ops/developer/auditor/readonly） |
|---|---|---|---|
| website:site:list | 站点列表 | website | a / o / d / au / r |
| website:site:read | 站点详情 | website | a / o / d / au / r |
| website:site:create | 创建站点 | website | a / o / d / - / - |
| website:site:update | 编辑站点与域名 | website | a / o / d / - / - |
| website:site:delete | 删除站点（敏感） | website | a / o / d / - / - |
| website:site:exec | 启停站点与重载服务（敏感） | website | a / o / d / - / - |
| website:config:read | 查看站点配置 | website | a / o / d / au / r |
| website:config:update | 编辑站点配置与回滚（敏感） | website | a / o / d / - / - |
| website:setting:read | 查看站点设置 | website | a / o / d / au / r |
| website:setting:update | 修改站点设置（默认文档、缓存、防盗链、访问限制、重定向、限速、日志开关） | website | a / o / d / - / - |
| website:rewrite:read | 查看伪静态规则 | website | a / o / d / au / r |
| website:rewrite:update | 修改伪静态规则（敏感，直接下发 Nginx 片段） | website | a / o / d / - / - |
| website:log:read | 查看站点访问/错误日志 | website | a / o / d / au / r |
| website:log:exec | 清空站点日志（敏感，不可撤销） | website | a / o / d / - / - |
| website:ssl:read | 规划。查看 SSL 配置 | website | a / o / d / au / r |
| website:ssl:update | 规划。绑定证书与强制跳转 | website | a / o / d / - / - |
| website:proxy:update | 规划。独立的反向代理规则管理 | website | a / o / d / - / - |
| website:log:export | 规划。导出网站日志 | website | a / o / d / au / - |
| website:backup:list | 规划。站点备份列表 | website | a / o / d / au / r |
| website:backup:create | 规划。立即备份站点 | website | a / o / d / - / - |
| website:service:read | 规划。Nginx 主配置与运行状态 | website | a / o / d / au / r |
| website:service:update | 规划。修改 Nginx 主配置 | website | a / o / - / - / - |
...
| file:file:read | 读取文件内容/预览/下载 | file | a / o / d / au / r |
| file:file:create | 新建文件/目录/上传 | file | a / o / d / - / - |
| file:file:update | 编辑/重命名/移动 | file | a / o / d / - / - |
| file:file:delete | 删除文件（含回收站清空） | file | a / o / d / - / - |
| file:file:exec | 压缩/解压 | file | a / o / d / - / - |
| file:permission:update | 修改文件权限与属主（敏感，需二次确认） | file | a / o / - / - / - |
| file:file:export | 打包下载与导出（未实现；单文件下载归 file:file:read） | file | a / o / d / au / - |
| file:recycle:list | 回收站列表（未实现） | file | a / o / d / au / r |
| file:recycle:update | 恢复回收站文件（未实现） | file | a / o / d / - / - |
| file:share:list | 文件分享列表（未实现） | file | a / o / d / au / r |
| file:share:create | 创建分享链接（未实现） | file | a / o / d / - / - |
| file:share:delete | 取消分享（未实现） | file | a / o / d / - / - |
...
| key | 分组 | 默认值 | 类型 | 加密 | 说明 |
|---|---|---|---|---|---|
| panel.name | basic | 青垣面板 | string | 否 | 面板显示名称 |
...
写串行化：SQLite 单库仅允许一个写者，青垣面板 因此拆分**双连接池**——
...
青垣面板 采用**共享库共享表 + `tenant_id` 行级隔离**，而非分库或分 Schema，理由：面板定位为私有化部署工具，绝大多数场景为单租户（`tenant_id=0`），少数场景（服务商为客户分区、企业内多事业部）需要轻量隔离；分库会使 SQLite 默认形态失效、迁移与备份复杂度成倍上升。
