...
| 项目 | 内容 |
| --- | --- |
| 产品名称 | 青垣面板（英文 Qingyuan Panel） |
| 文档版本 | v1.0 |
| 更新日期 | 2026-08-17 |
| 适用版本 | 青垣面板 v1.0 |
| 状态 | 定稿 |

青垣面板 是一套面向 Linux 服务器的运维管理面板，覆盖网站与运行环境托管、文件与终端、容器与编排、多节点集群纳管、监控告警、安全合规审计、应用商店与备份恢复。相比宝塔面板一类的单机工具，青垣面板 的差异化集中在四处：**多节点集群管理**、**容器与编排**、**监控告警与可观测**、**安全与合规审计**。
