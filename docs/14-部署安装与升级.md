...
青垣面板 只交付一套二进制，形态差异全部由 `panel.yaml` 与是否部署 `nova-agent` 决定，不存在「社区版包／企业版包」之类的分叉产物。
...
被控节点：无任何入向端口需求；出向需可达控制面 34568，以及按需可达备份存储与镜像源。这一点是 青垣面板 与「面板反向连接被控端」类方案的核心差异，安全组策略上被控节点可保持零入站。
...
check_ports() {
  ...
  for p in "$PANEL_PORT" "$AGENT_PORT"; do
    ...
    if [[ "$occupant" == *novapanel* || "$occupant" == *nova-agent* ]]; then
      log "端口 ${p} 由已有 青垣面板 进程占用，重装模式下正常"
    else
...
init_config() {
  ...
  cat >"$conf" <<EOF
# 青垣面板 配置文件，由 install.sh ${SCRIPT_VERSION} 于 $(date '+%F %T') 生成
# 全量配置项说明见文档 14.8
server:
  host: 0.0.0.0
  port: ${PANEL_PORT}
...
```ini
[Unit]
Description=Qingyuan Panel Control Plane (青垣面板控制面)
...
```ini
[Unit]
Description=Qingyuan Panel Agent (青垣面板被控端)
Documentation=https://docs.novapanel.io/deploy
After=network-online.target time-sync.target
Wants=network-online.target
StartLimitIntervalSec=300
...
FROM debian:12-slim
LABEL org.opencontainers.image.title="Qingyuan Panel" \
...
# 青垣面板 v1.0 · 200 节点档参考配置。未列出的键使用内置默认值。
server:
...
404 响应必须与 Nginx 默认 404 无法区分：不带 `Server` 头、不带自定义响应头、响应体为空。目的是让扫描器无法通过响应特征识别出这里跑着 青垣面板。
...
if ! $YES; then
  echo "即将卸载 青垣面板。数据目录处理方式: $($PURGE && echo '彻底删除（不可恢复）' || echo '保留')"
