...
```gotemplate
{{define "static.conf"}}# Qingyuan managed file. site={{.Site.Name}} id={{.Site.ID}} version={{.Meta.Version}}
...
```gotemplate
{{define "upstream.conf"}}# Qingyuan managed upstream={{.Upstream.Name}} version={{.Meta.Version}}
...
```gotemplate
{{define "ssl.conf"}}# Qingyuan managed TLS for {{.Site.Name}} cert={{.SSL.CertID}} expire={{.SSL.NotAfter}}
...
第一期已落地的能力统一走 `GET/PUT /website/sites/{id}/settings`（整体替换语义，渲染后 `nginx -t` 通过才 reload），下表「接口」列保留规划时设想的细粒度路径，实际实现见 [API接口规范](./05-API接口规范.md) 5.7.3。
...
| 限速 | 已实现，`settings.rateLimit` | 主配置 | `limit_rate` + `limit_rate_after`；`limit_conn`/`limit_req` 需 http 块 zone，暂不暴露 |
| 防盗链 | 已实现，`settings.antiLeech` | 主配置 | `valid_referers` + `$invalid_referer` 判定，可选允许空 Referer |
...
| 默认文档 | 已实现，`settings.indexFiles` | 主配置 | `index` 指令，顺序即优先级 |
| 运行目录 | 已实现，`settings.runPath` | 主配置 | 相对站点根的子目录，越界与不存在都会被拒绝 |
| IP / UA 限制 | 已实现，`settings.accessControl` | 主配置 | `allow`/`deny` 支持 CIDR；UA 按不区分大小写正则返回 403 |
| 301 重定向 | 已实现，`settings.redirects` | 主配置 | 支持整站/路径，`keepPath`/`keepQuery` 开关；反代站点禁止整站跳转 |
| CORS | `PUT /website/sites/{id}/cors` | 主配置 | `add_header ... always` + OPTIONS 快速返回 204 |
| 日志开关 | 已实现，`settings.accessLogEnabled` / `errorLogEnabled` | 主配置 | 关闭写 `access_log off` |
| 日志查看与清空 | 已实现，`GET /website/sites/{id}/logs`、`POST .../logs/clear` | `/www/wwwlogs/<site>.log` | 只读尾部（默认 200 行、上限 2000 行/512KiB）；清空用 truncate 保留 inode，路径被限制在日志目录内 |
| 维护模式 | `PUT /website/sites/{id}/maintenance` | 主配置 | IP 白名单 + 503 页；页面模板可自定义 |
| 目录浏览 | 已实现，`settings.autoIndex` | 主配置 | `autoindex on` + `autoindex_exact_size off`，默认关闭并在界面提示风险 |
| 静态缓存 | 已实现，`settings.cacheProfile` | 主配置 | `expires` 档位（不缓存/1 小时/1 天/30 天/1 年） |
...
icon: openresty.svg
description: 基于 Nginx 的高性能 Web 平台，青垣面板 的默认 Web 服务
versions:
  - version: "1.27.1.1"
    default: true
    lts: true
...
| 7.5 配置模板与下发 | `GET/PUT /website/sites/{id}/config`、`POST /website/sites/{id}/config/rollback/{version}`、`POST /website/nginx/test|reload` | `website:config:read/update`、`website:site:exec` |
| 7.6 伪静态 | `GET /website/rewrite/templates`、`GET/PUT /website/sites/{id}/rewrite` | `website:rewrite:read/update` |
| 7.7 反向代理与负载均衡 | `PUT /website/sites/{id}`（type=proxy/lb 时的 upstream 字段） | `website:site:update` |
| 7.8 站点安全策略 | 第一期聚合到 `GET/PUT /website/sites/{id}/settings`（防盗链、IP/UA 限制、重定向、限速、缓存、默认文档） | `website:setting:read/update` |
| 7.9 SSL 与 ACME | `GET/PUT /website/sites/{id}/ssl`、`/cert/certs`、`/cert/orders`、`/cert/accounts`、`/cert/dns-providers` | `cert:*` |
| 7.10 日志与切割 | 已实现：`GET /website/sites/{id}/logs`（尾部读取）、`POST /website/sites/{id}/logs/clear`；切割策略待后续 | `website:log:read/exec` |
